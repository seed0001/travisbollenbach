...
Room 02 is the Shore, at slash rooms slash beach — the game's exhale. Step through and it is daytime: a warm cove with rolling animated surf, low-poly palms leaning off the dunes, umbrellas, loungers, a beach ball, and procedural wave-wash audio that never repeats. Nothing hunts you. Ten shells are scattered along the wet sand; each pays five XP and gathering all ten clears the room for a hundred XP and fifty points. Walking works like the lobby — W A S D, mouse look, escape frees the mouse. The world spec lives in source, lib, beach dot T S; the scene in source, components, BeachRoom dot T S X.

Room 01 is the Galaxy, at slash rooms slash galaxy. It knows who's flying: a census in the corner shows how many pilots are aboard right now, split between desktop and mobile — every room connects to the same presence server as the lobby, each room its own channel. On a phone the room flies AR style: choose motion controls at the door and the ship aims where you point the phone, with two thumb buttons — hold thrust to fly, tap fire for the phaser. Asteroids tumble between the planets, and the phaser pops them. On desktop the mouse steers through pointer lock — escape frees the mouse, click returns to flight — with W and S for throttle, shift to boost, click or space to fire. There is no stand-in ship: the real fighter streams in, and the HUD says "your fighter is inbound" until it lands. Step through the gate and there is no floor: you are given a star fighter — a real 3D model, credited asset — in open space with a sun and nine seeded uncharted planets, some ringed, each with a procedural name. Twelve shards of light drift near the planets. Fly with the mouse (the ship steers toward your cursor), throttle with W and S, boost with shift; on a phone, the right half of the screen steers and the left is the throttle. Every shard collected pays five XP, and bringing home all twelve clears the room for a hundred XP and fifty points. The world spec lives in source, lib, space dot T S; the scene in source, components, GalaxyRoom dot T S X; the ship model at public, models, star fighter dot G L B.
