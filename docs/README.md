...
├── blue pill → /storefront          The Portfolio Walk (the GitHub project gallery)
...
| [The Portfolio Walk](03-portfolio-walk.md) | The blue pill: a 3D boulevard of 53 GitHub projects in twelve categories |
...
Most visitor-facing copy lives in [`src/lib/content.ts`](../src/lib/content.ts) — names, taglines, essay text, and signage are edited there, not in components. The one exception is the Portfolio Walk's project list: its categories and repo links live in the `CATEGORIES` array in [`src/components/PortfolioWalk.tsx`](../src/components/PortfolioWalk.tsx).
